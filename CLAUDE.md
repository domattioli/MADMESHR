...
- Boundary distance filter (3% fan radius, session 12) is NOT the cause of rectangle regression — the "0.426" result was from wrong domain definitions in eval scripts (3.0×1.5 instead of 4.0×1.0). Rectangle baseline is stable at q=0.464. Filter threshold now configurable via `bnd_dist_threshold` parameter (default 0.03).
